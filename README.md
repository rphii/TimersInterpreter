...
2. The second slightly less riskier version involves the unique trait of named [[#Scopes]]. Those types of comments are quite situational. You can treat a string as a comment, denoted by surrounded apostrophes (those guys: `'`). _We're skipping quite a bit, maybe come back later if you understood scopes._
...
- The safest way to comment out things is by taking advantage of the fact that it doesn't make sense to destroy one timer two times in succession, we're talking of the [[#Operations|operation]] `~`. So, by using two `~~` right next to each other, anything on that line afterwards is treated as a comment. Note that unlike in the programming language C, you can't escape the end of line with a `\`, so you have to effectively mark each line as a comment, if you wish to do so.
...
### Operations
The following operations can be put inside any [[#Time Functions]].
...
|`d`        |Execute a defined scope, where `d` gets replaced with the name (see [[#Scopes]])|
|`{f}`      |Execute a scope, where `f` gets replaced with function(s) of time. Similar to above, but inlined|
|`[t]`      |Initiate a new timer (finish execution of current function), starting from `t`, where `t` is a number (see [[#New]])|
...
### Modifiers
Modifiers can be used in [[#New]] or for [[#Time Functions]].
...
- [[#Time Functions]]: `1 2()` is equivalent to the function of time `2()`, where the `1` gets discarded. If you _really_ want to either hit `1` or `2`, you have to concatenate those in this case: `1|2()`.
